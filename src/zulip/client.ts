...
export function assertSuccess(payload: ZulipApiResponse, context: string): void {
...
export async function zulipClientRequestWithRetry<T>(
  client: ZulipClient,
  path: string,
  init?: RequestInit,
  options?: {
...
> {
  ...
  const timeout = setTimeout(() => controller.abort(), timeoutMs + 15000);
  try {
    return await zulipClientRequestWithRetry<
      ZulipApiResponse & { events?: Array<{ id: number; type: string; message?: ZulipMessage }> }
    >(
      client,
      `/events?${qs.toString()}`,
...
export async function sendZulipStreamMessageViaClient(
  client: ZulipClient,
  params: {
    stream: string;
    topic: string;
...
export async function uploadZulipFileViaClient(
  client: ZulipClient,
  filePath: string,
): Promise<{ url: string }> {
  ...
  const payload = await zulipClientRequestWithRetry<ZulipApiResponse & { uri?: string }>(
    client,
    "/user_uploads",
    {
      method: "POST",
...
export async function addZulipReactionViaClient(
  client: ZulipClient,
  params: {
    messageId: string;
    emojiName: string;
...
export async function removeZulipReactionViaClient(
  client: ZulipClient,
  params: {
    messageId: string;
    emojiName?: string;
...
export async function updateZulipRealm(client: ZulipClient, params: ZulipRealmUpdate): Promise<void> {
  const body = new URLSearchParams();
