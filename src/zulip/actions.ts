  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    if (action === "send") {
      ...
      if (target.kind === "stream") {
        const result = await sendZulipStreamMessageViaClient(client, {
    ...
    if (action === "poll") {
      ...
      const target = splitStreamTarget(targetRaw);
    ...
    if (action === "react") {
      ...
      if (remove) {
        await removeZulipReactionViaClient(client, {
          messageId,
      ...
      await addZulipReactionViaClient(client, {
        messageId,
        emojiName: emojiName ?? "",
        emojiCode: emojiCode ?? undefined,
        reactionType: reactionType ?? undefined,
